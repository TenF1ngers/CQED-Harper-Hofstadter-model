...
class Interpreter:
  ...
  def PlotInterference(self, calculations, experiment_config, num_atoms):
    ...
    for i in range(len(site_monitor_id_list)):
      data_full.append([calculations[phi_id][i] for phi_id in \
                        range(len(experiment_config['phi_list'][0]) - 1, -1, -1)])

    fig, axes = plt.subplots(1, len(site_monitor_id_list),
                             figsize=(2 * len(site_monitor_id_list), 2), sharey=True, dpi=200)
    axes = np.array(axes).reshape(len(site_monitor_id_list))

    axes[0].set_ylabel(r"Flux [$\Phi_0$]")

    for i in range(axes.shape[0]):
      image = axes[i].pcolormesh(experiment_config['operating_time_points'],
                                 experiment_config['phi_list'][0], data_full[i], cmap='inferno')
    ...
    cbar.set_ticks([0, np.max(data_full[axes.shape[0] - 1]) / 2,
                    np.max(data_full[axes.shape[0] - 1])])
    ...
    plt.tight_layout()
    plt.savefig(f"Results/Interference-{num_atoms}q-{len([site for t in \
                experiment_config['chosen_sites'] for site in t])}-\
                [{experiment_config['site_exc_id'][0]}-\
                {experiment_config['site_monitor_id_list'][-1]}]-{timestamp}.png",
                bbox_inches='tight')


  def PlotLocalization(self, calculations, experiment_config, num_atoms):
    fig, axes = plt.subplots(2, len(calculations), figsize=(len(calculations) * 2, 4), sharey=True,
                             sharex=True, dpi=150)
    ...
    for idx in range(len(calculations)):
      ...
      ax = axes[0, idx]
      # PuRd plt.cm.RdBu_r seismic
      # image = ax.pcolormesh(data, cmap='bwr', norm=LogNorm(vmin=1e-6, vmax=1))
      image = ax.pcolormesh(data, cmap='bwr')
      if (idx == (len(calculations) - 1)):
        image_last_top = image
      ax.set_xlabel("Site [№]", fontsize=10)
      ax.set_xticklabels([])
      ax.set_yticks([0, experiment_config['operating_time_points'][-1] // 2,
                     experiment_config['operating_time_points'][-1]])
      ax.text(0, experiment_config['operating_time_points'][-1] * 1.05,
              f"F = {experiment_config['ef_strength_list'][idx]:.2f}J")
    ...
    plt.tight_layout()
    plt.savefig(f"Results/Localization-{num_atoms}q-{len(experiment_config['chosen_sites'])}-\
                {experiment_config['site_exc_id']}_{timestamp}.png", bbox_inches='tight')
  ...
  def PlotDeflection(self, calculations, experiment_config):
    if (experiment_config['launch_code'] == 4):
      height = 4
      width = 4
      calculations = np.array(calculations).reshape(len(experiment_config['ef_strength_list']),
                                                    height, width, -1)

      x_avgs_ef_depend_rotated = []
      y_avgs_ef_depend_rotated = []
      x_time_avgs = np.zeros((len(experiment_config['ef_strength_list']),
                              len(experiment_config['operating_time_points'])))
      y_time_avgs = np.zeros((len(experiment_config['ef_strength_list']),
                              len(experiment_config['operating_time_points'])))
      ...
      for ef_id, result in enumerate(calculations):        
        ...
        for t in range(len(experiment_config['operating_time_points'])):
          x_time_avgs[ef_id, t] = 1 / (t + 1) * \
                                      np.trapz(x_avgs_t[:(t + 1)],
                                               experiment_config['operating_time_points'][:(t + 1)])
          y_time_avgs[ef_id, t] = 1 / (t + 1) * \
                                      np.trapz(y_avgs_t[:(t + 1)],
                                               experiment_config['operating_time_points'][:(t + 1)])
      ...
      colors = [cmap((ef_strength - experiment_config['ef_strength_list'].min()) /
                     (experiment_config['ef_strength_list'].max() -
                      experiment_config['ef_strength_list'].min()))
                      for ef_strength in experiment_config['ef_strength_list']]

      ax = axes[0][0]
      for i, color in enumerate(colors):
        ax.plot(x_time_avgs[i], y_time_avgs[i], color=color,
                label=f"{experiment_config['ef_strength_list'][i]:.2f}")
      ax.grid(ls=':')
      ...
      ax = axes[0][1]
      for i, color in enumerate(colors):
        ax.plot(experiment_config['operating_time_points'], x_time_avgs[i], color=color,
                label=f"{experiment_config['ef_strength_list'][i]:.2f}")
      ax.grid(ls=':')
      ...
      ax = axes[0][2]
      for i, color in enumerate(colors):
        ax.plot(experiment_config['operating_time_points'], y_time_avgs[i], color=color,
                label=f"{experiment_config['ef_strength_list'][i]:.2f}")
      ax.grid(ls=':')
      ax.set_xlabel("t [ns]", fontsize=15)
      ax.set_ylabel(r"$\langle \overline{y} \rangle$", fontsize=15)
      ...
      ax = axes[1][0]
      for i, color in enumerate(colors):
        ax.plot(x_avgs_ef_depend_rotated[i], y_avgs_ef_depend_rotated[i], color=color,
                label=f"{experiment_config['ef_strength_list'][i]:.2f}")
      ax.grid(ls=':')
      ...
      ax = axes[1][1]
      for i, color in enumerate(colors):
        ax.plot(experiment_config['operating_time_points'], x_avgs_ef_depend_rotated[i],
                color=color, label=f"{experiment_config['ef_strength_list'][i]:.2f}")
      ax.grid(ls=':')
      ax.set_xlabel("t [ns]", fontsize=15)
      ...
      ax = axes[1][2]
      for i, color in enumerate(colors):
        ax.plot(experiment_config['operating_time_points'], y_avgs_ef_depend_rotated[i],
                color=color, label=f"{experiment_config['ef_strength_list'][i]:.2f}")
      ax.grid(ls=':')
      ax.set_xlabel("t [ns]", fontsize=15)
      ax.set_ylabel(r"$\langle y \rangle$", fontsize=15)

      ax.legend(title="ef_strength", fontsize=15, bbox_to_anchor=(1.05, 0.5), loc="center left",
                borderaxespad=0)
      ...
      y_time_avgs = np.zeros((len(experiment_config['ef_strength_list']),
                              len(experiment_config['phi_list'])))
      ...
      for ef_id, result_ef_fixed in enumerate(calculations):
        for phi_id, result_ef_phi_fixed in enumerate(result_ef_fixed):
          ...
          y_time_avgs[ef_id, phi_id] = \
                  1 / len(experiment_config['operating_time_points']) * \
      ...
      for phi_id in range(len(experiment_config['phi_list'])):
        ...
        p0 = [
          (np.max(data) - np.min(data)) / (experiment_config['ef_strength_list'][-1] -
                                           experiment_config['ef_strength_list'][0]),
          0
        ]
        popt, _ = curve_fit(FitLin, experiment_config['ef_strength_list'], data, p0=p0,
                            maxfev=10000)
      ...
      cmap = plt.get_cmap("coolwarm")
      colors_ef = [cmap((ef_strength - experiment_config['ef_strength_list'].min()) /
                        (experiment_config['ef_strength_list'].max() -
                         experiment_config['ef_strength_list'].min()))
                         for ef_strength in experiment_config['ef_strength_list']]
      ...
      for i, color in enumerate(colors_ef):
        ax.plot(experiment_config['phi_list'], y_time_avgs[i, :], color=color,
                label=f"{experiment_config['ef_strength_list'][i]:.2f}")
      ax.grid(ls=':')
      ...
      colors_mf = [cmap((phi - experiment_config['phi_list'].min()) /
                        (experiment_config['phi_list'].max() -
                         experiment_config['phi_list'].min()))
                         for phi in experiment_config['phi_list']]
      ax = axes[1]
      for i, color in enumerate(colors_mf):
        ax.scatter(experiment_config['ef_strength_list'], y_time_avgs[:, i], color=color,
                   label=f"{experiment_config['phi_list'][i]:.2f}")
        ax.plot(experiment_config['ef_strength_list'], FitLin(experiment_config['ef_strength_list'],
                                                              *popts[i]), color=color)
      ax.grid(ls=':')
      ax.set_xlabel("Electric field", fontsize=15)
      ax.set_ylabel(r"$\langle \overline{y} \rangle$", fontsize=15)
      ax.legend(title="Flux", fontsize=15, bbox_to_anchor=(1.05, 0.5), loc="center left",
                borderaxespad=0)
